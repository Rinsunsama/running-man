...
public class KinectManager : MonoBehaviour
{
    ...
    // How high off the ground is the sensor (in meters).  设备放置的高度
    public float sensorHeight = 1.0f;

    // Public Bool to determine whether to receive and compute the user map 是否捕获深度数据
    public bool computeUserMap = false;

    // Public Bool to determine whether to receive and compute the color map 是否捕获彩色数据
    public bool computeColorMap = false;

    // Public Bool to determine whether to receive and compute the infrared map 是否捕捉红外数据
    public bool computeInfraredMap = false;

    // Public Bool to determine whether to display user map on the GUI 是否在GUI上展示深度图像
    public bool displayUserMap = false;

    // Public Bool to determine whether to display color map on the GUI 是否在GUI上展示色彩图像
    public bool displayColorMap = false;

    // Public Bool to determine whether to display the skeleton lines on user map 是否在GUI上展示骨骼线
    public bool displaySkeletonLines = false;

    // Public Floats to specify the width and height of the depth and color maps as % of the camera width and height //在镜头中显示的尺寸
    ...
    // Minimum user distance in order to process skeleton data 摄用户离摄像机最近的距离
    public float minUserDistance = 0.5f;

    // Public Bool to determine whether to detect only the closest user or not是否只检测最近的用户
    public bool detectClosestUser = true;

    // Public Bool to determine whether to use only the tracked joints (and ignore the inferred ones)是否只要骨骼信息忽略红外信息
    public bool ignoreInferredJoints = true;

    // Selection of smoothing parameters
    public enum Smoothing : int { None, Default, Medium, Aggressive }//挑选平滑度
    public Smoothing smoothing = Smoothing.Default;

    // Public Bool to determine the use of additional filters
    public bool useBoneOrientationConstraints = true;//额外的过滤器
    //public bool useBoneOrientationsFilter = false;

    // Lists of AvatarController-objects that will be controlled by Kinect users 
    public List<AvatarController> avatarControllers;//avaterController控制物体的列表

    // Calibration pose for each player, if needed // 每位用户标准的姿势
    public KinectGestures.Gestures playerCalibrationPose;

    // List of Gestures to be detected for each player//每位要被探测的用户的手势
    public List<KinectGestures.Gestures> playerCommonGestures;

    // Minimum time between gesture detections//手势之间的最短时间
    public float minTimeBetweenGestures = 0.7f;

    // List of Gesture Listeners. They must implement KinectGestures.GestureListenerInterface//手势监听器
    public List<MonoBehaviour> gestureListeners;

    // GUI Text to show messages.//标准的显示测试信息的text
    ...
    // Bool to keep track of whether Kinect has been initialized一直检测Kinect是否被初始化
    private bool kinectInitialized = false;

    // The singleton instance of KinectManager kinectManager的单独实例
    private static KinectManager instance = null;

    //	// KinectServer instance
    //	private KinectServer kinectServer = null;

    // SensorData structure  检测到的信息
    ...
    private Color32[] usersHistogramImage;//使用柱状图,深度缓冲
    private ushort[] usersPrevState;//用户身体数据的缓冲
    private float[] usersHistogramMap;//柱状信息

    private Texture2D usersLblTex;//用户的骨骼信息
    private Rect usersMapRect;//深度坐标
    private int usersMapSize;//地图的大小
    //	private int minDepth;
    //	private int maxDepth;

    // Color map
    //private KinectInterop.ColorBuffer colorImage;
    private Texture2D usersClrTex;//用户彩色信息
    private Rect usersClrRect;//用户彩色信息坐标
    private int usersClrSize;//尺寸

    // Kinect body frame data
    private KinectInterop.BodyFrameData bodyFrame;//交互的骨骼的帧数
    //private Int64 lastBodyFrameTime = 0;

    // List of all users  所有用户的ID
    private List<Int64> alUserIds;
    private Dictionary<Int64, int> dictUserIdToIndex;

    // Primary (first or closest) user ID 最靠近的用户的ID
    private Int64 liPrimaryUserId = 0;

    // Kinect to world matrix  世界坐标系下
    ...
    // gestures data and parameters 手势数据和参数
    ...
    // List of Gesture Listeners. They must implement KinectGestures.GestureListenerInterface 手势监听事件
    public List<KinectGestures.GestureListenerInterface> gestureListenerInts;

    // Body filter instances
    private JointPositionsFilter jointPositionFilter = null;//骨骼位置过滤器
    private BoneOrientationsConstraint boneConstraintsFilter = null;//骨骼方向
    //private BoneOrientationsFilter boneOrientationFilter = null;
    public GameObject video_plane;  //显示图像的地方
    // returns the single KinectManager instance
    public GameObject zanTing;
    public static KinectManager Instance//返回kinectmanager的实例
    {
        get
        {
            return instance;
    ...
    public static bool IsKinectInitialized()//检测Kinect manager是否被实例化
    {
        return instance != null ? instance.kinectInitialized : false;
    }

    // checks if Kinect is initialized and ready to use. If not, there was an error during Kinect-sensor initialization
    public bool IsInitialized()//检测是否被实例化
    {
    ...
    public Int64 GetPrimaryUserID()//获取到主要用户的ID
    {
    ...
    public KinectInterop.BodyData GetUserBodyData(Int64 userId)//获取到用户身体的数据，但仅仅用于Debug
    {
    ...
    public Vector3 GetUserPosition(Int64 userId)//获取到用户的位置
    {
        if (dictUserIdToIndex.ContainsKey(userId))
        {
            int index = dictUserIdToIndex[userId];
    ...
    public Quaternion GetUserOrientation(Int64 userId, bool flip)//返回用户的rotation
    {
        if (dictUserIdToIndex.ContainsKey(userId))
        {
            int index = dictUserIdToIndex[userId];
    ...
    public TrackingState GetJointTrackingState(Int64 userId, int joint)//返回用户的骨骼跟踪信息
    {
        if (dictUserIdToIndex.ContainsKey(userId))
        {
            int index = dictUserIdToIndex[userId];
    ...
    public bool IsJointTracked(Int64 userId, int joint)//判断用户的骨骼是否被Tracked到
    {
        if (dictUserIdToIndex.ContainsKey(userId))
        {
            int index = dictUserIdToIndex[userId];
    ...
    public Vector3 GetJointPosition(Int64 userId, int joint)//返会用户的骨骼位置
    {
        if (dictUserIdToIndex.ContainsKey(userId))
        {
            int index = dictUserIdToIndex[userId];
    ...
    public Vector3 GetJointDirection(Int64 userId, int joint, bool flipX, bool flipZ)//返会用户的骨骼位置
    {
        if (dictUserIdToIndex.ContainsKey(userId))
        {
            int index = dictUserIdToIndex[userId];
    ...
    public Vector3 GetDirectionBetweenJoints(Int64 userId, int firstJoint, int secondJoint, bool flipX, bool flipZ)//返会用户两个关节点的位置
    {
        if (dictUserIdToIndex.ContainsKey(userId))
        {
            int index = dictUserIdToIndex[userId];
    ...
    public Quaternion GetJointOrientation(Int64 userId, int joint, bool flip)//获取到关节点的方向
    {
        if (dictUserIdToIndex.ContainsKey(userId))
        {
            int index = dictUserIdToIndex[userId];
    ...
    public bool IsLeftHandConfidenceHigh(Int64 userId)//检测用户左手是否高举
    {
        if (dictUserIdToIndex.ContainsKey(userId))
        {
            int index = dictUserIdToIndex[userId];
    ...
    // returns true if the confidence is high, false if it is low or user is not found
    public bool IsRightHandConfidenceHigh(Int64 userId)//检测用户右手是否高举
    {
        if (dictUserIdToIndex.ContainsKey(userId))
        {
            int index = dictUserIdToIndex[userId];
    ...
    public HandState GetLeftHandState(Int64 userId)//获取到用户左手的状态
    {
        if (dictUserIdToIndex.ContainsKey(userId))
        {
            int index = dictUserIdToIndex[userId];
    ...
    public HandState GetRightHandState(Int64 userId)//获取用户右手的状态
    {
        if (dictUserIdToIndex.ContainsKey(userId))
        {
            int index = dictUserIdToIndex[userId];
    ...
    public bool GetLeftHandInteractionBox(Int64 userId, ref Vector3 leftBotBack, ref Vector3 rightTopFront, bool bValidBox)//获取用户左手交互的盒子
    {
        if (dictUserIdToIndex.ContainsKey(userId))
        {
            int index = dictUserIdToIndex[userId];
    ...
    public bool GetRightHandInteractionBox(Int64 userId, ref Vector3 leftBotBack, ref Vector3 rightTopFront, bool bValidBox)//获取用户右手交互的盒子
    {
        if (dictUserIdToIndex.ContainsKey(userId))
        {
            int index = dictUserIdToIndex[userId];
    ...
    public void ClearKinectUsers()//移除目前的用户，允许新的用户进来
    {
    ...
    // resets data filters
    public void ResetFilters()//重设过滤器
    {
    ...
    public void DetectGesture(Int64 UserId, KinectGestures.Gestures gesture)//检测用户手势
    {
    ...
    public bool ResetGesture(Int64 UserId, KinectGestures.Gestures gesture)//重置用户手势
    {
    ...
    public void ResetPlayerGestures(Int64 UserId)//重置用户手势函数重载
    {
        List<KinectGestures.GestureData> gesturesData = playerGesturesData.ContainsKey(UserId) ? playerGesturesData[UserId] : null;
    ...
    public bool DeleteGesture(Int64 UserId, KinectGestures.Gestures gesture)//删除用户手势
    {
        List<KinectGestures.GestureData> gesturesData = playerGesturesData.ContainsKey(UserId) ? playerGesturesData[UserId] : null;
        int index = gesturesData != null ? GetGestureIndex(gesture, ref gesturesData) : -1;
        if (index < 0)
    ...
    public void ClearGestures(Int64 UserId)//清除用户手势
    {
        List<KinectGestures.GestureData> gesturesData = playerGesturesData.ContainsKey(UserId) ? playerGesturesData[UserId] : null;
    ...
    public int GetGesturesCount(Int64 UserId)//获取手势编号
    {
        List<KinectGestures.GestureData> gesturesData = playerGesturesData.ContainsKey(UserId) ? playerGesturesData[UserId] : null;
    ...
    public List<KinectGestures.Gestures> GetGesturesList(Int64 UserId)//返回用户手势的手势的列表
    {
    ...
    public bool IsGestureDetected(Int64 UserId, KinectGestures.Gestures gesture)//判断用户的手势是否被探测到
    {
        List<KinectGestures.GestureData> gesturesData = playerGesturesData.ContainsKey(UserId) ? playerGesturesData[UserId] : null;
        int index = gesturesData != null ? GetGestureIndex(gesture, ref gesturesData) : -1;
        return index >= 0;
    ...
    public bool IsGestureComplete(Int64 UserId, KinectGestures.Gestures gesture, bool bResetOnComplete)//判断用户的手势是否完成
    {
        List<KinectGestures.GestureData> gesturesData = playerGesturesData.ContainsKey(UserId) ? playerGesturesData[UserId] : null;
        int index = gesturesData != null ? GetGestureIndex(gesture, ref gesturesData) : -1;
    ...
    public bool IsGestureCancelled(Int64 UserId, KinectGestures.Gestures gesture)//判断用户的手势是否取消
    {
        List<KinectGestures.GestureData> gesturesData = playerGesturesData.ContainsKey(UserId) ? playerGesturesData[UserId] : null;
        int index = gesturesData != null ? GetGestureIndex(gesture, ref gesturesData) : -1;
    ...
    public float GetGestureProgress(Int64 UserId, KinectGestures.Gestures gesture)//返回用户手势的进程
    {
        List<KinectGestures.GestureData> gesturesData = playerGesturesData.ContainsKey(UserId) ? playerGesturesData[UserId] : null;
        int index = gesturesData != null ? GetGestureIndex(gesture, ref gesturesData) : -1;
    ...
    public Vector3 GetGestureScreenPos(Int64 UserId, KinectGestures.Gestures gesture)//用户在屏幕上手势的位置
    {
        List<KinectGestures.GestureData> gesturesData = playerGesturesData.ContainsKey(UserId) ? playerGesturesData[UserId] : null;
        int index = gesturesData != null ? GetGestureIndex(gesture, ref gesturesData) : -1;
    ...
    void OnGUI()//在屏幕上显示信息
    {
        if (kinectInitialized)
        {
            ...
            if (computeColorMap && displayColorMap)
            {
                ...
                video_plane.renderer.material.mainTexture = usersClrTex;
            } //*/ // by zg
        }
    }
    ...
    void Update() //更新
    {
        if (kinectInitialized)
        {
            if (computeColorMap)
            {
                ...
                {
                    UpdateColorMap();
                    //	video_plane.renderer.material.mainTexture = usersClrTex;  //by zg
                }
            }
            if (computeUserMap)
            {
    ...
    void UpdateInfraredMap()//更新红外数据
    {
        // does nothing at the moment
    }

    // Update the user histogram map
    void UpdateUserHistogramImage()//更新用户的柱状图
    {
        int numOfPoints = 0;
    ...
    private void ProcessBodyFrameData()//帧数数据进行过程
    {
    ...
    void CalibrateUser(Int64 userId, int bodyIndex)//添加用户
    {
    ...
    void RemoveUser(Int64 userId)//移除用户
    {
        int uidIndex = alUserIds.IndexOf(userId);
        Debug.Log("Removing user " + uidIndex + ", ID: " + userId);//添加游戏暂停。
        Time.timeScale = 0;
    ...
    private void DrawSkeleton(Texture2D aTexture, ref KinectInterop.BodyData bodyData)//在制定的2dTecture描绘出用户的骨骼
    {
    ...
    private void DrawLine(Texture2D a_Texture, int x1, int y1, int x2, int y2, Color a_Color)//画出一条线
    {
    ...
    private Quaternion ConvertMatrixToQuat(Quaternion rotation, int joint, bool flip)//将给定的矩阵转换成Quaternion
    {
    ...
    private void SetJointOrientations(ref KinectInterop.BodyData bodyData)//计算给定身体的骨骼的方向
    {
    ...
    private void CheckForGestures(Int64 UserId)//估计目前的状态所代表的姿势
    {
    ...
    private bool IsConflictingGestureInProgress(KinectGestures.GestureData gestureData, ref List<KinectGestures.GestureData> gesturesData)//在进程中姿势是否进行干扰
    {
    ...
    private int GetGestureIndex(KinectGestures.Gestures gesture, ref List<KinectGestures.GestureData> gesturesData)//返回列表中姿势的数据
    {
        int listSize = gesturesData.Count;
    ...
    private bool CheckForCalibrationPose(Int64 UserId, int bodyIndex, KinectGestures.Gestures calibrationGesture)//检测用户的姿势是否完成
    {
